...
class AutoCropper(object):
    ...
    def eval_rois_batch(self, images, rois_batch):
        """
        批量推理接口 (真正的GPU batch)
        :param images: List[np.ndarray], 每张图是 HWC RGB 格式
        :param rois_batch: List[List[roi]], 每张图的 ROI 列表
        :return: List[List[int]], 每张图的候选框排序索引
        """
        # 1. 图像预处理
        tensor_list = []
        for img in images:
            img_t = transpose_image_op(img)  # HWC → CHW
            tensor_list.append(torch.as_tensor(img_t))
        batch_tensor = torch.stack(tensor_list, dim=0).float()  # (B, C, H, W)

        # 2. 拼接所有 ROI，加 batch_index
        all_rois = []
        for b_idx, rois in enumerate(rois_batch):
            for roi in rois:
                all_rois.append([b_idx, *roi])  # (batch_index, x1, y1, x2, y2)
        all_rois = torch.tensor(all_rois, dtype=torch.float32, device=batch_tensor.device)

        if self.cuda:
            batch_tensor = batch_tensor.cuda()
            all_rois = all_rois.cuda()

        # 3. 一次性前向传播
        with torch.no_grad():
            out = self.cropper(batch_tensor, all_rois)

        # 4. 按 batch_index 拆分结果
